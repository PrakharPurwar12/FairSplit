...
class MemberAnalyticsView(APIView):
    ...
    def get(self, request, member_id):
        ...
        if not assignments.exists():
            ...
            return Response({"message": "No tasks assigned."})
...
class RiskAnalyticsView(APIView):
    ...
    def get(self, request, project_id):
        ...
        if total_tasks:
            high_risk_percentage = round((high_count / total_tasks) * 100, 2)

        high_risk_tasks = []

        high_tasks = tasks.filter(predicted_risk="High").order_by("-risk_confidence")
