...
def project_dashboard(project_id):
    ...
    total_members = ProjectMember.objects.filter(project=project).count()
...
def skill_analytics(project_id):
    ...
    skills = (
        TaskSkill.objects.filter(task__project_id=project_id)
        .values("skill__name")
        .annotate(usage=Count("skill"))
        .order_by("-usage")
    )
...
def assignment_history(project_id):
    ...
    history = (
        AssignmentHistory.objects.filter(task__project_id=project_id)
        .select_related("task", "previous_member", "new_member")
        .order_by("-changed_at")
    )
...
def top_contributors(project_id):
    ...
    contributors = (
        TaskAssignment.objects.filter(task__project_id=project_id)
        .values("assigned_to__username")
        .annotate(assigned_tasks=Count("id"))
