...
def get_current_workloads(project):
    ...
    for assignment in assignments:
        ...
        workloads[assignment.assigned_to_id] += float(assignment.task.estimated_hours)
...
def calculate_fairness_penalty(member):
    ...
    assignment_count = TaskAssignment.objects.filter(assigned_to=member.user).count()
...
def calculate_confidence(best_score, second_best_score):
    ...
    confidence = ((best_score - second_best_score) / best_score) * 100
...
def generate_reason(
    ...
    if penalty > 0:
        reasons.append(f"Fairness penalty applied ({penalty})")

    reasons.append("Highest final score among eligible members")
...
def allocate_tasks(project_id):
    ...
    if not members.exists():
        return {"error": "No project members found."}

    workloads = get_current_workloads(tasks.first().project)
    ...
    for task in tasks:
        ...
        for member in members:
            ...
            skill_score, matched_skills = calculate_skill_score(member, task)

            workload_score = calculate_workload_score(workloads[member.user.id])

            penalty = calculate_fairness_penalty(member)

            final_score = calculate_final_score(skill_score, workload_score, penalty)
        ...
        second_score = candidates[1]["final_score"] if len(candidates) > 1 else 0
...
def build_prediction_features(task):
    ...
    # Required skills
    required_skills = TaskSkill.objects.filter(task=task).count()

    # Active tasks
    active_tasks = TaskAssignment.objects.filter(
        assigned_to=member.user, task__status__in=["todo", "progress", "review"]
    ).count()
...
def recommend_reassignment(task):
    ...
    if not hasattr(task, "assignment"):
        return {"error": "Task is not assigned."}
    ...
    if not candidates:
        ...
        return {"error": "No eligible replacement found."}
