    ...
    >
      <div className="w-full flex items-center justify-between px-6">
        ...
        <div className="hidden md:flex items-center gap-8 font-medium text-sm text-gray-600 dark:text-text-secondary">
          ...
          <Link to={isAuthenticated ? '/dashboard' : '/login'} className="hover:text-gray-900 dark:hover:text-text transition-colors">Dashboard</Link>
        </div>

        {/* CTA Buttons */}
        <div className="flex items-center gap-3">
          <ThemeToggle />
          <Link to={signInPath} className="text-sm font-semibold text-gray-700 dark:text-text hover:text-gray-900 dark:hover:text-white transition-colors px-4 py-2 rounded-full bg-white/50 dark:bg-surface border border-white/60 dark:border-border shadow-sm dark:shadow-none hover:bg-white/80 dark:hover:bg-surface-2">
