...
const SocialButtons = () => {
  ...
  const handleOAuthClick = async (provider) => {
    ...
    try {
      ...
      if (res.url) {
        ...
        setError(`Could not initiate ${provider} OAuth authentication.`);
      }
    } catch (err) {
      console.error(`OAuth initiation error (${provider}):`, err);
      setError(err.response?.data?.error || `Failed to connect with ${provider}. Please check configuration.`);
    } finally {
  ...
  return (
    <div className="flex flex-col gap-3">
      {error && (
        <div className="p-3 bg-red-50 dark:bg-red-950/20 text-red-600 dark:text-red-400 text-xs rounded-xl border border-red-200 dark:border-red-900/50 flex items-center gap-2">
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <motion.button
          ...
          whileHover={{ scale: loadingProvider ? 1 : 1.01, y: loadingProvider ? 0 : -1 }}
          whileTap={{ scale: loadingProvider ? 1 : 0.98 }}
          className="flex-1 flex items-center justify-center gap-3 bg-white dark:bg-[#1C1C1E] border border-border hover:border-gray-300 dark:hover:border-gray-600 text-text font-medium py-3 px-4 rounded-xl transition-all shadow-sm hover:shadow-md disabled:opacity-50 disabled:cursor-not-allowed"
        >
